...
while(1):
    ...
    #serial write
    X = 640		#hard coded position values for testing purposes
    Y = 480
    PUSHER = (X * 1000)  + Y	#combine x and y in the form xxxyyy
    PUSHER = str(PUSHER)		#convert value to string
    PUSHER = format(PUSHER, "0>6") #"0>6" command ensures message is always 6 characters long and padded with 0s
    
    #ser.write(1);
    ser.write(str(999).encode()) #send tag of 999 at the front of the message
