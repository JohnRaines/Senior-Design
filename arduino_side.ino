...
//
//relevant section: beginning of update fans routine
//
void setup() {
  pinMode(FANA, OUTPUT);
  pinMode(FANB, OUTPUT);
...
void updateFans(String x) {
  ...
  if (xcord == 0 && ycord == 0) return;        //return early if garbage packet (0,0 is not valid)
  if (x.substring(0,3).toInt() != 999) return; //return if the 999 tag is not there
  if (x.length() != 9) return;				   //return if message is not the right length
